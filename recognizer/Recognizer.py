...
class RecognizationService:
    ...
    def save_all_current_image(cls, user_id):
        for root, dirs, files in os.walk(fr'{PATH}\current_image'):
            for filename in files:
                old_loc = os.path.join(root, filename)
                split_filename = filename.split('_')
                split_filename = split_filename[2].split('.')
                new_loc = fr'{PATH}\users_images\user_{user_id}\image_{split_filename[0]}.jpg'
                os.replace(old_loc, new_loc)

    @classmethod
    def save_current_image(cls, user_id):
        os.makedirs(fr'{PATH}\users_images\user_{user_id}')

        old_loc = cls.current_image_path
        new_loc = fr'{PATH}\images_base\{user_id}.jpg'
        shutil.copyfile(old_loc, new_loc)

        cls.save_all_current_image(user_id)
        cls.face_train()
        return True
    ...
    def delete_image(cls, user_id):
        ...
        shutil.rmtree(fr'{PATH}\users_images\user_{user_id}')
        cls.face_train()
        return True
    ...
    def check_image(cls):
        ...
        for encode_face, face_loc in zip(encodes_cur_frame, faces_cur_frame):
            ...
            if matches[match_index]:
                name = user_names_list[match_index]
            else:
                name = None
            return name
